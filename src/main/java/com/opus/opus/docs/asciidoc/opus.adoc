ifndef::snippets[]
:snippets: ./build/generated-snippets
endif::[]
= 🚀 SW프로젝트관리시스템(OPUS) API
:doctype: book
:icons: font
:source-highlighter: highlightjs
:toc: left
:toclevels: 3
:sectnums:

== 멤버 관련 API

link:./member.html[회원 API]

== 팀 관련 API

link:./team.html[팀 API]

link:./team-comment.html[팀 댓글 API]
link:./team-vote.html[팀 투표 API]

link:./team-member.html[팀원 API]

link:./team-like.html[팀 좋아요 API]

== 공지 관련 API

link:./notice.html[공지 API]

== 알림 관련 API

link:./notification.html[알림 API]

== 대회 관련 API

link:./contest.html[대회 API]

link:./contest-category.html[대회 카테고리 API]

link:./contest-track.html[대회 분과 API]

link:./contest-submission.html[제출물 API]

link:./contest-submission-feedback.html[제출물 피드백 API]

link:./contest-member.html[대회 배정(교수/외부멘토) API]

link:./contest-submission-item.html[대회 제출 항목 API]

link:./submission-file.html[제출 파일 다운로드 API]

link:./contest-team-submission.html[제출 현황 요약/타임라인 API]

link:./mentor.html[멘토 활동 조회 API]
