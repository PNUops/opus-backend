ifndef::snippets[]
:snippets: ./build/generated-snippets
endif::[]

= CONTEST API 문서
:doctype: book
:icons: font
:source-highlighter: highlightjs
:toc: left
:toclevels: 3
:sectnums:

== API 목록

link:./opus.html[API 목록으로 돌아가기]

== 투표 기간 관련
=== `GET`: 투표 기간 조회

.HTTP Request
include::{snippets}/get-vote-period/http-request.adoc[]

.HTTP Response
include::{snippets}/get-vote-period/http-response.adoc[]

.Path Parameters
include::{snippets}/get-vote-period/path-parameters.adoc[]

.Response Fields
include::{snippets}/get-vote-period/response-fields.adoc[]


=== `PUT`: 투표 기간 수정

.HTTP Request
include::{snippets}/update-vote-period/http-request.adoc[]

.HTTP Response
include::{snippets}/update-vote-period/http-response.adoc[]

.Path Parameters
include::{snippets}/update-vote-period/path-parameters.adoc[]

.Request Headers
include::{snippets}/update-vote-period/request-headers.adoc[]

.Request Fields
include::{snippets}/update-vote-period/request-fields.adoc[]


== 최대 투표 개수 관리

=== `PATCH`: 최대 투표 개수 설정

.Path Parameters
include::{snippets}/update-max-votes-limit/path-parameters.adoc[]

.HTTP Request Headers
include::{snippets}/update-max-votes-limit/request-headers.adoc[]

.HTTP Request
include::{snippets}/update-max-votes-limit/http-request.adoc[]

.HTTP Response
include::{snippets}/update-max-votes-limit/http-response.adoc[]

.Request Fields
include::{snippets}/update-max-votes-limit/request-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 존재하지 않는 대회

[%collapsible]

====

include::{snippets}/update-max-votes-limit-fail-not-found/http-request.adoc[]

include::{snippets}/update-max-votes-limit-fail-not-found/http-response.adoc[]

====

.❌ Case 2: 투표 진행 중

[%collapsible]

====

include::{snippets}/update-max-votes-limit-fail-voting-period/http-request.adoc[]

include::{snippets}/update-max-votes-limit-fail-voting-period/http-response.adoc[]

====

=== `GET`: 최대 투표 개수 조회

.Path Parameters
include::{snippets}/get-max-votes-limit/path-parameters.adoc[]

.HTTP Request Headers
include::{snippets}/get-max-votes-limit/request-headers.adoc[]

.HTTP Request
include::{snippets}/get-max-votes-limit/http-request.adoc[]

.HTTP Response
include::{snippets}/get-max-votes-limit/http-response.adoc[]

.Response Body's Fields
include::{snippets}/get-max-votes-limit/response-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 존재하지 않는 대회

[%collapsible]

====

include::{snippets}/get-max-votes-limit-fail-not-found/http-request.adoc[]

include::{snippets}/get-max-votes-limit-fail-not-found/http-response.adoc[]

====

== 대회 배너 이미지 관리

=== `GET`: 대회 배너 이미지 조회

.Path Parameters
include::{snippets}/get-contest-banner/path-parameters.adoc[]

.HTTP Request
include::{snippets}/get-contest-banner/http-request.adoc[]

.HTTP Response
include::{snippets}/get-contest-banner/http-response.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 존재하지 않는 대회

[%collapsible]
====
include::{snippets}/get-contest-banner-fail-contest-not-found/http-request.adoc[]
include::{snippets}/get-contest-banner-fail-contest-not-found/http-response.adoc[]
====

.❌ Case 2: 등록되지 않은 이미지

[%collapsible]
====
include::{snippets}/get-contest-banner-fail-image-not-found/http-request.adoc[]
include::{snippets}/get-contest-banner-fail-image-not-found/http-response.adoc[]
====

.❌ Case 3: 변환 중인 이미지

[%collapsible]
====
include::{snippets}/get-contest-banner-fail-converting/http-request.adoc[]
include::{snippets}/get-contest-banner-fail-converting/http-response.adoc[]
====

=== `POST`: 대회 배너 이미지 등록

.Path Parameters
include::{snippets}/save-contest-banner/path-parameters.adoc[]

.Request Headers
include::{snippets}/save-contest-banner/request-headers.adoc[]

.Request Parts
include::{snippets}/save-contest-banner/request-parts.adoc[]

.HTTP Request
include::{snippets}/save-contest-banner/http-request.adoc[]

.HTTP Response
include::{snippets}/save-contest-banner/http-response.adoc[]

=== `DELETE`: 대회 배너 이미지 삭제

.Path Parameters
include::{snippets}/delete-contest-banner/path-parameters.adoc[]

.Request Headers
include::{snippets}/delete-contest-banner/request-headers.adoc[]

.HTTP Request
include::{snippets}/delete-contest-banner/http-request.adoc[]

.HTTP Response
include::{snippets}/delete-contest-banner/http-response.adoc[]

== 정렬 관리

=== `PUT`: 대회 정렬 설정 변경

.Path Parameters
include::{snippets}/update-contest-sort/path-parameters.adoc[]

.HTTP Request Headers
include::{snippets}/update-contest-sort/request-headers.adoc[]

.HTTP Request
include::{snippets}/update-contest-sort/http-request.adoc[]

.HTTP Response
include::{snippets}/update-contest-sort/http-response.adoc[]

.Request Fields
include::{snippets}/update-contest-sort/request-fields.adoc[]

=== `GET`: 대회 정렬 방식 조회

.Path Parameters
include::{snippets}/get-contest-sort/path-parameters.adoc[]

.HTTP Request Headers
include::{snippets}/get-contest-sort/request-headers.adoc[]

.HTTP Request
include::{snippets}/get-contest-sort/http-request.adoc[]

.HTTP Response
include::{snippets}/get-contest-sort/http-response.adoc[]

.Response Fields
include::{snippets}/get-contest-sort/response-fields.adoc[]

=== `PUT`: 대회 수동 정렬 순서 저장

.Path Parameters
include::{snippets}/update-contest-sort-custom/path-parameters.adoc[]

.HTTP Request Headers
include::{snippets}/update-contest-sort-custom/request-headers.adoc[]

.HTTP Request
include::{snippets}/update-contest-sort-custom/http-request.adoc[]

.HTTP Response
include::{snippets}/update-contest-sort-custom/http-response.adoc[]

.Request Fields
include::{snippets}/update-contest-sort-custom/request-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: CUSTOM 모드가 아님

[%collapsible]
====
include::{snippets}/update-contest-sort-custom-fail-mode/http-request.adoc[]
include::{snippets}/update-contest-sort-custom-fail-mode/http-response.adoc[]
====

.❌ Case 2: request에 중복된 teamId 존재

[%collapsible]
====
include::{snippets}/update-contest-sort-custom-fail-duplicate-teamId/http-request.adoc[]
include::{snippets}/update-contest-sort-custom-fail-duplicate-teamId/http-response.adoc[]
include::{snippets}/update-contest-sort-custom-fail-duplicate-teamId/request-fields.adoc[]
====

.❌ Case 3: request에 중복된 itemOrder 존재

[%collapsible]
====
include::{snippets}/update-contest-sort-custom-fail-duplicate-itemOrder/http-request.adoc[]
include::{snippets}/update-contest-sort-custom-fail-duplicate-itemOrder/http-response.adoc[]
include::{snippets}/update-contest-sort-custom-fail-duplicate-itemOrder/request-fields.adoc[]
====

.❌ Case 4: 요청 팀 개수와 저장된 팀 개수 다름

[%collapsible]
====
include::{snippets}/update-contest-sort-custom-fail-different-size/http-request.adoc[]
include::{snippets}/update-contest-sort-custom-fail-different-size/http-response.adoc[]
====

.❌ Case 5: itemOrder가 팀 개수를 넘어감

[%collapsible]
====
include::{snippets}/update-contest-sort-custom-fail-over-itemOrder/http-request.adoc[]
include::{snippets}/update-contest-sort-custom-fail-over-itemOrder/http-response.adoc[]
====

== 대회 관리

=== `POST`: 대회 생성

.HTTP Request Headers
include::{snippets}/create-contest/request-headers.adoc[]

.HTTP Request
include::{snippets}/create-contest/http-request.adoc[]

.HTTP Response
include::{snippets}/create-contest/http-response.adoc[]

.Request Fields
include::{snippets}/create-contest/request-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 동일한 대회명 존재

[%collapsible]

====

include::{snippets}/create-contest-fail/http-request.adoc[]

include::{snippets}/create-contest-fail/http-response.adoc[]

include::{snippets}/create-contest-fail/request-fields.adoc[]

====

=== `PATCH`: 대회 수정

NOTE: 대회 생성과 동일하게 이미 저장되어 있는 대회 이름은 저장 불가

.HTTP Request Headers
include::{snippets}/update-contest/request-headers.adoc[]

.Path Parameters
include::{snippets}/update-contest/path-parameters.adoc[]

.HTTP Request
include::{snippets}/update-contest/http-request.adoc[]

.HTTP Response
include::{snippets}/update-contest/http-response.adoc[]

.Request Fields
include::{snippets}/update-contest/request-fields.adoc[]

=== `DELETE`: 대회 삭제

.HTTP Request Headers
include::{snippets}/delete-contest/request-headers.adoc[]

.Path Parameters
include::{snippets}/delete-contest/path-parameters.adoc[]

.HTTP Request
include::{snippets}/delete-contest/http-request.adoc[]

.HTTP Response
include::{snippets}/delete-contest/http-response.adoc[]

=== `GET`: 대회 목록 조회

.HTTP Request
include::{snippets}/get-all-contest/http-request.adoc[]

.HTTP Response
include::{snippets}/get-all-contest/http-response.adoc[]

.Response Body's Fields
include::{snippets}/get-all-contest/response-fields.adoc[]

== 투표 로그 관리

=== `GET`: 투표 로그 조회

.HTTP Request Headers
include::{snippets}/get-contest-vote-log/request-headers.adoc[]

.Path Parameters
include::{snippets}/get-contest-vote-log/path-parameters.adoc[]

.HTTP Request
include::{snippets}/get-contest-vote-log/http-request.adoc[]

.HTTP Response
include::{snippets}/get-contest-vote-log/http-response.adoc[]

.Response Fields
include::{snippets}/get-contest-vote-log/response-fields.adoc[]

== 프로젝트 등록 현황 조회

=== `GET`: 대회 팀별 프로젝트 등록 현황 조회

.Path Parameters
include::{snippets}/get-team-submissions/path-parameters.adoc[]

.Request Headers
include::{snippets}/get-team-submissions/request-headers.adoc[]

.HTTP Request
include::{snippets}/get-team-submissions/http-request.adoc[]

.HTTP Response
include::{snippets}/get-team-submissions/http-response.adoc[]

.Response Fields
include::{snippets}/get-team-submissions/response-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 존재하지 않는 대회

[%collapsible]

====

include::{snippets}/get-team-submissions-fail-not-found/path-parameters.adoc[]

include::{snippets}/get-team-submissions-fail-not-found/http-request.adoc[]

include::{snippets}/get-team-submissions-fail-not-found/http-response.adoc[]

====

== 투표 랭킹 조회

=== `GET`: 대회 투표 랭킹 조회

.Path Parameters
include::{snippets}/get-team-ranking/path-parameters.adoc[]

.Request Headers
include::{snippets}/get-team-ranking/request-headers.adoc[]

.HTTP Request
include::{snippets}/get-team-ranking/http-request.adoc[]

.HTTP Response
include::{snippets}/get-team-ranking/http-response.adoc[]

.Response Fields
include::{snippets}/get-team-ranking/response-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 존재하지 않는 대회

[%collapsible]

====

include::{snippets}/get-team-ranking-fail-not-found/path-parameters.adoc[]

include::{snippets}/get-team-ranking-fail-not-found/http-request.adoc[]

include::{snippets}/get-team-ranking-fail-not-found/http-response.adoc[]

====

== 투표 집계 조회

=== `GET`: 대회 투표 집계 조회

.Path Parameters
include::{snippets}/get-vote-statistics/path-parameters.adoc[]

.Request Headers
include::{snippets}/get-vote-statistics/request-headers.adoc[]

.HTTP Request
include::{snippets}/get-vote-statistics/http-request.adoc[]

.HTTP Response
include::{snippets}/get-vote-statistics/http-response.adoc[]

.Response Fields
include::{snippets}/get-vote-statistics/response-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 존재하지 않는 대회

[%collapsible]

====

include::{snippets}/get-vote-statistics-fail-not-found/path-parameters.adoc[]

include::{snippets}/get-vote-statistics-fail-not-found/http-request.adoc[]

include::{snippets}/get-vote-statistics-fail-not-found/http-response.adoc[]

====

== 대회 팀 템플릿 설정 관리

=== `GET`: 대회의 템플릿 조회

.Path Parameters
include::{snippets}/get-contest-template/path-parameters.adoc[]

.HTTP Request
include::{snippets}/get-contest-template/http-request.adoc[]

.HTTP Response
include::{snippets}/get-contest-template/http-response.adoc[]

.Response Fields
include::{snippets}/get-contest-template/response-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 존재하지 않는 대회

[%collapsible]
====
include::{snippets}/get-contest-template-fail-not-found/path-parameters.adoc[]
include::{snippets}/get-contest-template-fail-not-found/http-request.adoc[]
include::{snippets}/get-contest-template-fail-not-found/http-response.adoc[]
====

=== `PUT`: 대회의 템플릿 수정

.Path Parameters
include::{snippets}/update-contest-template/path-parameters.adoc[]

.Request Headers
include::{snippets}/update-contest-template/request-headers.adoc[]

.Request Fields
include::{snippets}/update-contest-template/request-fields.adoc[]

.HTTP Request
include::{snippets}/update-contest-template/http-request.adoc[]

.HTTP Response
include::{snippets}/update-contest-template/http-response.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 존재하지 않는 대회

[%collapsible]
====
include::{snippets}/update-contest-template-fail-not-found/path-parameters.adoc[]
include::{snippets}/update-contest-template-fail-not-found/http-request.adoc[]
include::{snippets}/update-contest-template-fail-not-found/http-response.adoc[]
====

== 대회 전체 팀 조회

=== `GET`: 대회의 팀 목록 조회 (비회원용)

NOTE: 비회원용 메인 페이지 API입니다. `isLiked`와 `isVoted` 값이 항상 `false` 입니다.

.Path Parameters
include::{snippets}/get-contest-team-summaries-public/path-parameters.adoc[]

.HTTP Request
include::{snippets}/get-contest-team-summaries-public/http-request.adoc[]

.HTTP Response
include::{snippets}/get-contest-team-summaries-public/http-response.adoc[]

.Response Body's Fields
include::{snippets}/get-contest-team-summaries-public/response-fields.adoc[]

=== `GET`: 대회의 팀 목록 조회 (회원용 - 미투표 기간)

NOTE: 회원만 접근 가능한 메인 페이지용 API입니다. 미투표 기간에는 로그인한 사용자의 `isLiked` 여부가 포함됩니다.

.Path Parameters
include::{snippets}/get-contest-team-summaries-member-non-voting/path-parameters.adoc[]

.Request Headers
include::{snippets}/get-contest-team-summaries-member-non-voting/request-headers.adoc[]

.HTTP Request
include::{snippets}/get-contest-team-summaries-member-non-voting/http-request.adoc[]

.HTTP Response
include::{snippets}/get-contest-team-summaries-member-non-voting/http-response.adoc[]

.Response Body's Fields
include::{snippets}/get-contest-team-summaries-member-non-voting/response-fields.adoc[]

=== `GET`: 대회의 팀 목록 조회 (회원용 - 투표 기간)

NOTE: 회원만 접근 가능한 메인 페이지용 API입니다. 투표 기간에는 로그인한 사용자의 `isVoted` 여부가 포함됩니다.

.Path Parameters
include::{snippets}/get-contest-team-summaries-member-voting/path-parameters.adoc[]

.Request Headers
include::{snippets}/get-contest-team-summaries-member-voting/request-headers.adoc[]

.HTTP Request
include::{snippets}/get-contest-team-summaries-member-voting/http-request.adoc[]

.HTTP Response
include::{snippets}/get-contest-team-summaries-member-voting/http-response.adoc[]

.Response Body's Fields
include::{snippets}/get-contest-team-summaries-member-voting/response-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 존재하지 않는 대회 ID

[%collapsible]
====
.HTTP Request
include::{snippets}/get-contest-team-summaries-fail-contest-not-found/http-request.adoc[]

.HTTP Response
include::{snippets}/get-contest-team-summaries-fail-contest-not-found/http-response.adoc[]

.Path Parameters
include::{snippets}/get-contest-team-summaries-fail-contest-not-found/path-parameters.adoc[]
====

== 팀 일괄 등록 (엑셀)

=== `POST`: 엑셀 파일로 팀 일괄 등록

NOTE: 관리자 권한이 필요합니다. 모든 팀이 유효성 검사를 통과해야만 등록되며, 하나라도 실패하면 전체가 등록되지 않습니다.

.Path Parameters
include::{snippets}/bulk-upload-teams/path-parameters.adoc[]

.Request Headers
include::{snippets}/bulk-upload-teams/request-headers.adoc[]

.Request Parts
include::{snippets}/bulk-upload-teams/request-parts.adoc[]

.HTTP Request
include::{snippets}/bulk-upload-teams/http-request.adoc[]

.HTTP Response
include::{snippets}/bulk-upload-teams/http-response.adoc[]

.Response Fields
include::{snippets}/bulk-upload-teams/response-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 잘못된 파일 형식

[%collapsible]
====
include::{snippets}/bulk-upload-teams-fail-invalid-format/http-request.adoc[]
include::{snippets}/bulk-upload-teams-fail-invalid-format/http-response.adoc[]
====

.❌ Case 2: 빈 엑셀 파일 (데이터 없음)

[%collapsible]
====
include::{snippets}/bulk-upload-teams-fail-empty/http-request.adoc[]
include::{snippets}/bulk-upload-teams-fail-empty/http-response.adoc[]
====

.❌ Case 3: 유효성 검사 실패

[%collapsible]
====
include::{snippets}/bulk-upload-teams-fail-validation/http-request.adoc[]
include::{snippets}/bulk-upload-teams-fail-validation/http-response.adoc[]
include::{snippets}/bulk-upload-teams-fail-validation/response-fields.adoc[]
====

.❌ Case 4: 존재하지 않는 대회

[%collapsible]
====
include::{snippets}/bulk-upload-teams-fail-contest-not-found/http-request.adoc[]
include::{snippets}/bulk-upload-teams-fail-contest-not-found/http-response.adoc[]
====

== 팀 대시보드 요약 조회

=== `GET`: 팀 대시보드 요약 조회

NOTE: 팀 소속 학생 또는 관리자만 접근 가능합니다. 미제출 항목 수, 가장 가까운 마감 일시, 읽지 않은 피드백 수, 최근 피드백 미리보기를 반환합니다.

.Path Parameters
include::{snippets}/get-team-dashboard-summary/path-parameters.adoc[]

.Request Headers
include::{snippets}/get-team-dashboard-summary/request-headers.adoc[]

.HTTP Request
include::{snippets}/get-team-dashboard-summary/http-request.adoc[]

.HTTP Response
include::{snippets}/get-team-dashboard-summary/http-response.adoc[]

.Response Fields
include::{snippets}/get-team-dashboard-summary/response-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 존재하지 않는 대회

[%collapsible]
====
include::{snippets}/get-team-dashboard-summary-fail-contest-not-found/http-request.adoc[]
include::{snippets}/get-team-dashboard-summary-fail-contest-not-found/http-response.adoc[]
====

.❌ Case 2: 존재하지 않는 팀

[%collapsible]
====
include::{snippets}/get-team-dashboard-summary-fail-team-not-found/http-request.adoc[]
include::{snippets}/get-team-dashboard-summary-fail-team-not-found/http-response.adoc[]
====

.❌ Case 3: 소속되지 않은 팀

[%collapsible]
====
include::{snippets}/get-team-dashboard-summary-fail-not-team-member/http-request.adoc[]
include::{snippets}/get-team-dashboard-summary-fail-not-team-member/http-response.adoc[]
====

== 관리자 제출 현황 조회

=== `GET`: 제출물 제출 현황 조회

선택한 제출 항목를 기준으로 전체 팀의 제출 현황을 조회합니다.
제출 상태는 저장값이 아니라 마감일시와 실제 제출 여부를 조회 시점에 비교하여 계산합니다.
제출 상태 · 분과 필터, 팀 이름 검색을 지원하며, 전체 데이터를 반환합니다.

.HTTP Request Headers
include::{snippets}/get-admin-contest-submissions/request-headers.adoc[]

.Path Parameters
include::{snippets}/get-admin-contest-submissions/path-parameters.adoc[]

.Query Parameters
include::{snippets}/get-admin-contest-submissions/query-parameters.adoc[]

.HTTP Request
include::{snippets}/get-admin-contest-submissions/http-request.adoc[]

.HTTP Response
include::{snippets}/get-admin-contest-submissions/http-response.adoc[]

.Response Fields
include::{snippets}/get-admin-contest-submissions/response-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 존재하지 않는 대회

[%collapsible]

====

include::{snippets}/get-admin-contest-submissions-fail-contest-not-found/http-request.adoc[]

include::{snippets}/get-admin-contest-submissions-fail-contest-not-found/http-response.adoc[]

include::{snippets}/get-admin-contest-submissions-fail-contest-not-found/path-parameters.adoc[]

====

=== `GET`: 제출물 제출 현황 통계 조회

제출 현황 상단 통계 카드(전체 / 제출 완료 / 미제출 / 지각 제출)를 채우기 위한 집계 정보를 조회합니다.
제출 항목 · 분과 필터를 기준으로 집계하며, 팀 이름 검색어는 통계에 반영하지 않습니다.
미제출 카드는 마감 전 미제출과 마감 후 미제출을 합산한 값으로 내려줍니다.

.HTTP Request Headers
include::{snippets}/get-admin-contest-submissions-summary/request-headers.adoc[]

.Path Parameters
include::{snippets}/get-admin-contest-submissions-summary/path-parameters.adoc[]

.Query Parameters
include::{snippets}/get-admin-contest-submissions-summary/query-parameters.adoc[]

.HTTP Request
include::{snippets}/get-admin-contest-submissions-summary/http-request.adoc[]

.HTTP Response
include::{snippets}/get-admin-contest-submissions-summary/http-response.adoc[]

.Response Fields
include::{snippets}/get-admin-contest-submissions-summary/response-fields.adoc[]

==== ⚠️ 실패 케이스

.❌ Case 1: 존재하지 않는 대회

[%collapsible]

====

include::{snippets}/get-admin-contest-submissions-summary-fail-contest-not-found/http-request.adoc[]

include::{snippets}/get-admin-contest-submissions-summary-fail-contest-not-found/http-response.adoc[]

include::{snippets}/get-admin-contest-submissions-summary-fail-contest-not-found/path-parameters.adoc[]

====
